...
ChiSquared4ParamsTrackletStyle::ChiSquared4ParamsTrackletStyle(const Settings* settings, const uint nPar) : L1ChiSquared(settings, nPar){
...
std::map<std::string, double> ChiSquared4ParamsTrackletStyle::vecToMap(std::vector<double> x){
...
std::vector<double> ChiSquared4ParamsTrackletStyle::mapToVec(std::map<std::string, double> x){
...
std::vector<double> ChiSquared4ParamsTrackletStyle::seed(const L1track3D& l1track3D){
...
Matrix<double> ChiSquared4ParamsTrackletStyle::D(std::vector<double> x){
    ...
    for(unsigned i = 0; i < stubs_.size(); i++){
        ...
        if(stubs_[i]->barrel()){
            D(j, 0) = -0.5*ri*ri/sqrt(1-0.25*ri*ri*rInv*rInv);
            D(j, 1) = ri;
            //D(j, 2) = 0;
            //D(j, 3) = 0;
            j++;
            ...
            double phi_track=phi0-0.5*rInv*(zi-z0)/t; //Expected phi hit given the track
            ...
            D(j, 0) = (phimultiplier*dphidrinv+rmultiplier*drdrinv);
            D(j, 1) = (phimultiplier*dphidphi0+rmultiplier*drdphi0);
            D(j, 2) = (phimultiplier*dphidt+rmultiplier*drdt);
            D(j, 3) = (phimultiplier*dphidz0+rmultiplier*drdz0);
            j++;
        }
    }
    return D;
...
Matrix<double> ChiSquared4ParamsTrackletStyle::Vinv(){
...
std::vector<double> ChiSquared4ParamsTrackletStyle::residuals(std::vector<double> x) {
   ...
   for(unsigned int i=0;i<n;i++) {
    ...
    if (print) std::cout << __FILE__ << ":" << __LINE__ << " - Residuals(): delta["<<j-2<<"]/delta["<<j-1<<"]: "<< delta[j-2] << "/" << delta[j-1] << std::endl; 
    if (print) std::cout << __FILE__ << ":" << __LINE__ << " - Residuals(): chisq: " << chiSq  << std::endl; 
...
std::map<std::string, double> ChiSquared4ParamsTrackletStyle::convertParams(std::vector<double> x){
...
std::string ChiSquared4ParamsTrackletStyle::getParams(){
    return "ChiSquared4ParamsTrackletStyle";
}
